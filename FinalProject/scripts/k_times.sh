...
./do_plan.sh light_up_domain.pddl example1_readable.pddl "wastar(blind(), w=$k)" | grep "Total time"
...
do
echo "wastar(gc(), w=$k)..."
./do_plan.sh light_up_domain.pddl example1_readable.pddl "wastar(gc(), w=$k)" | grep "Total time"
...
do
echo "wastar(blind(), w=$k)..."
./do_plan.sh light_up_domain.pddl example2_readable.pddl "wastar(blind(), w=$k)" | grep "Total time"
...
do
echo "wastar(gc(), w=$k)..."
./do_plan.sh light_up_domain.pddl example2_readable.pddl "wastar(gc(), w=$k)" | grep "Total time"
